...
# ============================================================
MODEL_PATH = "final_cifar10_model_exp6.keras"
GDRIVE_FILE_ID = "1eQ71ay40dkemH2wE0eIz_t6pyQ8YlyI3"
...
# ============================================================
#  GITHUB ICON (top-right, icon only)
# ============================================================
st.markdown(f"""
<a href="{GITHUB_URL}" target="_blank" class="github-corner">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
# ============================================================
#  MODEL DOWNLOAD + LOAD (cached)
# ============================================================
@st.cache_resource(show_spinner=False)
def get_model():
    """Download (if needed) and load the Keras model. Cached across reruns."""
...
def tta_predict(img_arr: np.ndarray) -> np.ndarray:
    """Run 4-fold Test-Time Augmentation and return shape (4, 10) probs."""
    base = img_arr.astype(np.uint8)
    pil  = Image.fromarray(base)
    augs = [
        base,
...
with st.sidebar:
    ...
    for i, cls in enumerate(CLASSES):
        with cols_sb[i % 2]:
            st.markdown(
                f"<div style='font-size:13px;margin-bottom:6px;'>{EMOJIS[cls]} {cls.capitalize()}</div>",
                unsafe_allow_html=True,
            )
    ...
    for k, v in [("Architecture", "ResNet-style CNN"), ("Dataset", "CIFAR-10"),
        ...
        st.markdown(
            ...
            f"margin-bottom:6px;'><span style='color:#6b7280;'>{k}</span>"
            f"<span style='font-family:DM Mono,monospace;color:#22d3ee;'>{v}</span></div>",
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        ...
        "Developed by <strong style='color:#6b7280;'>Md. Nafijul Islam</strong></div>",
        unsafe_allow_html=True,
    )
...
with st.expander("📈 Training History (Graphs)", expanded=False):
    ...
    with col_acc:
        ...
        fig_acc.update_layout(
            title="Accuracy over Epochs", xaxis_title="Epoch", yaxis_title="Accuracy (%)",
...
with right:
    ...
    if current_image is None:
        ...
        st.error("❌ Model not loaded. Please check the model file or Drive link.")
    else:
        img_resized = current_image.resize((32, 32))
        img_arr = np.array(img_resized, dtype=np.uint8)
        with st.spinner("Running TTA inference (4 folds)…"):
        ...
        for rank, i in enumerate(top5):
            ...
            <div class="prog-row">
                ...
                <div class="prog-track">
                    <div class="prog-fill" style="width:{v*100:.2f}%;
                         background:linear-gradient(90deg,{col}cc,{col}66);"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)

# ============================================================
#  ALL CLASSES & TTA BREAKDOWN (shown after upload)
# ============================================================
if file and current_image is not None and model is not None:
    st.divider()
    col_a, col_b = st.columns([1, 1], gap="large")
    ...
    with col_a:
        ...
        for i in sorted_idx:
            ...
            <div class="prog-row">
                ...
                <div class="prog-track">
                    <div class="prog-fill" style="width:{v*100:.2f}%;
                         background:linear-gradient(90deg,{col_c}cc,{col_c}44);"></div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        for k in range(4):
            ...
            with (c1 if k % 2 == 0 else c2):
                ...
                <div class="tta-card">
                    ...
                    <div class="tta-bar-wrap">
                        <div class="tta-bar-fill"
                             style="width:{pv*100:.1f}%;
                                    background:linear-gradient(90deg,{pcol},{pcol}88);">
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    <div class="glass-card">
        ...
        <div style="display:flex;align-items:center;gap:16px;">
            ...
            <div>
                ...
                <div style="font-size:12px;color:#6b7280;margin-top:2px;">
                    {'High confidence — all TTA folds are consistent.'
                     if agreement == 1 else
                     'Some folds disagree — treat confidence with caution.'}
                </div>
            </div>
            <div style="margin-left:auto;">
                <div style="font-family:'DM Mono',monospace;font-size:28px;
                            color:#22d3ee;font-weight:500;">{int(agreement * 100)}%</div>
            </div>
        </div>
        <div class="prog-track" style="margin-top:12px;height:8px;">
            <div class="prog-fill" style="width:{agreement*100:.0f}%;
                 background:{'linear-gradient(90deg,#10b981,#22d3ee)'
                             if agreement == 1 else
                             'linear-gradient(90deg,#f59e0b,#f87171)'};"></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
